...
def get_todo_stats(request: Request):
    ...
    Returns:
        dict: Comprehensive todo statistics
            - total_todos: Total number of todos
            - completed_todos: Number of completed todos
            - pending_todos: Number of pending todos
            - completion_rate: Percentage of completed todos
            - recent_activity: Todos created in last 7 days
            - avg_per_day: Average todos created per day
            - overdue_todos: Number of overdue todos
            - upcoming_due: Number of todos due in next 7 days
            
    Raises:
        HTTPException: 500 if database query fails
    """
    try:
        logger.info("📊 Todo statistics requested")
        conn = get_db_connection()
        cur = conn.cursor(cursor_factory=RealDictCursor)
        ...
        total_todos = sum(day['total_count'] for day in daily_stats)
        avg_per_day = round(total_todos / total_days, 2) if total_days > 0 else 0
        
        # Get due date statistics
        cur.execute("""
            SELECT 
                COUNT(*) FILTER (WHERE due_date < CURRENT_DATE AND completed = false) as overdue_count,
                COUNT(*) FILTER (WHERE due_date BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '7 days') as upcoming_count
            FROM todos 
            WHERE due_date IS NOT NULL
        """)
        due_stats = cur.fetchone()
        
        # Calculate completion rate
        completion_rate = 0
        if basic_stats['total_todos'] > 0:
            completion_rate = round((basic_stats['completed_todos'] / basic_stats['total_todos']) * 100, 1)
        
        cur.close()
        conn.close()
        
        stats = {
            "total_todos": basic_stats['total_todos'],
            "completed_todos": basic_stats['completed_todos'],
            "pending_todos": basic_stats['pending_todos'],
            "completion_rate": completion_rate,
            "recent_activity": {
                "last_7_days": recent_stats['recent_count'],
                "avg_per_day": avg_per_day
            },
            "due_dates": {
                "overdue_todos": due_stats['overdue_count'],
                "upcoming_due": due_stats['upcoming_count']
            },
            "daily_breakdown": [
                {
                    "date": str(day['created_date']),
                    "count": day['total_count']
                } for day in daily_stats[:7]  # Last 7 days
            ]
        }
        
        logger.info(f"✅ Todo statistics retrieved: {basic_stats['total_todos']} total, {completion_rate}% completion rate")
        return stats
